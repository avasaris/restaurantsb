...
public interface MenuRepository extends JpaRepository<Menu, Integer> {
    ...
    @Query("SELECT new ru.topjava.restaurant.to.VoteTo(m.restaurant.id, COUNT(v)) FROM Menu m LEFT OUTER JOIN Vote v ON m.id = v.menu.id WHERE m.date = :localDate GROUP BY m.restaurant.id HAVING COUNT(v) > 0")
